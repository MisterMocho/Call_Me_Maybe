...
class LLMEngine:
    ...
    def generate(self, prompt: str, max_tokens: int = 75) -> str:
        ...
        for step in range(max_tokens):
            # Tentamos aceder diretamente ao buffer de memória do Tensor
            if step == 0:
                next_token_id = next(iter(self.allowed_step_0))
            elif step == 1:
                next_token_id = next(iter(self.allowed_step_1))
            else:
                ...
                # --- FIM DO SKIPPING (Acordar a IA) ---
                    ...
                    last_struct = [c for c in generated_text if c in '{:,}']
                    if not last_struct or last_struct[-1] in '{,':
            ...
            open_brackets += clean_word.count('{')
            open_brackets -= clean_word.count('}')
